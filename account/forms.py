...
class UserRegistrationForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        if model.objects.filter(username_iexact=username).exists():
            raise forms.ValidationError('A Username already Exists')
        return username
    ...
    def clean_email(self):
        ...
        if model.objects.filter(email_iexact=email).exists():
...
class SendEmailForm(PasswordResetForm, threading.Thread):
    ...
    def clean_email(self):
        if not User.objects.filter(email_iexact=self.cleaned_data.get('email')).exists():
